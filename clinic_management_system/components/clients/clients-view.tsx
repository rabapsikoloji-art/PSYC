...
import { Plus, Edit, Trash2, Users, Search, Eye, Download, Upload, FileText, Brain } from "lucide-react";
...
import { PsychologistAssignment } from "@/components/clients/psychologist-assignment";
import { ComprehensiveAIAnalysis } from "@/components/clients/comprehensive-ai-analysis";
import {
  Table,
  TableBody,
  TableCell,
...
export function ClientsView() {
  ...
  const [isAnamnesisDialogOpen, setIsAnamnesisDialogOpen] = useState(false);
  const [selectedClient, setSelectedClient] = useState<ClientData | null>(null);
  const [viewClient, setViewClient] = useState<any | null>(null);
  const [deleteClientId, setDeleteClientId] = useState<string | null>(null);
  const [anamnesisClient, setAnamnesisClient] = useState<any | null>(null);
  const [isComprehensiveAIOpen, setIsComprehensiveAIOpen] = useState(false);
  const [aiAnalysisClient, setAIAnalysisClient] = useState<any | null>(null);
  ...
  return (
    <div className="space-y-6">
                    ...
                    <TableCell>
                      {client.assignedPsychologist ? (
                        <span className="text-sm">
                          {client.assignedPsychologist.firstName} {client.assignedPsychologist.lastName}
                        </span>
                      ) : (
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() => {
                            setAnamnesisClient(client);
                            setIsAnamnesisDialogOpen(true);
                          }}
                          title="Anamnez Formu"
                        >
          ...
          {viewClient && (
            <>
              <div className="flex justify-end">
                <Button
                  onClick={() => {
                    setAIAnalysisClient(viewClient);
                    setIsComprehensiveAIOpen(true);
                  }}
              ...
              <TabsList className="grid w-full grid-cols-4">
                ...
                <TabsTrigger value="psychologist">Psikolog</TabsTrigger>
              ...
              <TabsContent value="psychologist">
                <PsychologistAssignment
                  ...
                  clientName={`${viewClient.firstName} ${viewClient.lastName}`}
                  currentPsychologistId={viewClient.assignedPsychologistId}
                  onAssigned={() => {
                    openViewDialog(viewClient.id); // Refresh data
                  }}
                />
              </TabsContent>
      ...
      {/* Anamnez Formu Dialog */}
      <Dialog open={isAnamnesisDialogOpen} onOpenChange={setIsAnamnesisDialogOpen}>
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Anamnez Formu</DialogTitle>
            <DialogDescription>
              {anamnesisClient?.firstName} {anamnesisClient?.lastName} için anamnez formu
            </DialogDescription>
          </DialogHeader>
          {anamnesisClient && (
            <AnamnesisForm
              clientId={anamnesisClient.id}
              clientName={`${anamnesisClient.firstName} ${anamnesisClient.lastName}`}
              onSave={() => {
                setIsAnamnesisDialogOpen(false);
                toast.success("Anamnez formu kaydedildi");
              }}
            />
          )}
        </DialogContent>
      ...
      {/* Comprehensive AI Analysis Dialog */}
      {aiAnalysisClient && (
        <ComprehensiveAIAnalysis
          open={isComprehensiveAIOpen}
          onClose={() => {
            setIsComprehensiveAIOpen(false);
            setAIAnalysisClient(null);
          }}
          clientId={aiAnalysisClient.id}
          clientName={`${aiAnalysisClient.firstName} ${aiAnalysisClient.lastName}`}
        />
      )}
    </div>
  );
