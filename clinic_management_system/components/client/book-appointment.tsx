...
export function BookAppointment() {
    ...
    const loadData = async () => {
        try {
            const [clientRes, personnelRes, servicesRes, packagesRes] = await Promise.all([
                fetch("/api/client"),
                fetch("/api/personnel"),
                fetch("/api/services"),
            ...
            // Get client data including assigned psychologist
            let assignedPsychId: string | null = null;
            if (clientRes.ok) {
                const clientData = await clientRes.json();
                if (clientData.assignedPsychologistId) {
                    assignedPsychId = clientData.assignedPsychologistId;
                    setFormData(prev => ({ ...prev, personnelId: clientData.assignedPsychologistId }));
                }
            }

            if (personnelRes.ok) {
                const data = await personnelRes.json();
                // Filter only psychologists
                const psychologists = data.filter((p: any) => p.user?.role === "PSYCHOLOGIST");

                // If there's an assigned psychologist, only show that one
                if (assignedPsychId) {
                    const assignedPersonnel = psychologists.find((p: Personnel) => p.id === assignedPsychId);
                    ...
                    // Show all psychologists if no assignment
                    setPersonnel(psychologists);
                }
            }
            ...
            // Load available services for assigned psychologist
            if (assignedPsychId) {
                const psychServices = await fetch(`/api/personnel-services?personnelId=${assignedPsychId}`);
                if (psychServices.ok) {
                    const psData = await psychServices.json();
                    // Store which services this psychologist can provide
    ...
    // Update services when personnel changes
    useEffect(() => {
        const loadServicesForPersonnel = async () => {
            ...
            try {
                const psychServices = await fetch(`/api/personnel-services?personnelId=${formData.personnelId}`);
                if (psychServices.ok) {
                    const psData = await psychServices.json();
                    const serviceIds = psData.map((ps: any) => ps.serviceId);
                    ...
                    if (servicesRes.ok) {
                        ...
                        setServices(allServices.filter((s: Service) =>
                                ...
                                <div className="space-y-3">
                                    {personnel.length === 0 ? (
                                        ...
                                        personnel.map((p) => (
                                            <div
                                                key={p.id}
                                                className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all ${formData.personnelId === p.id
