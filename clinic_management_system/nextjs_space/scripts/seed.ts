...
async function main() {
  ...
  await prisma.personnelService.deleteMany();
  ...
  // 9. Create Integration Settings (Global API Keys)
  const aiIntegration = await prisma.integrationSettings.create({
    data: {
      integrationType: 'ai',
      providerName: 'AbacusAI',
      config: {
  ...
  const whatsappIntegration = await prisma.integrationSettings.create({
    data: {
      ...
      config: {
        ...
        accessToken: '',
        apiVersion: 'v18.0',
        clinicAddress: '19 Mayıs, Halaskargazi Cd. No:212 Kat:7 Daire:7, 34000 Şişli/İstanbul'
      },
      isActive: false // Initially inactive until user configures
    }
  });

  const googleIntegration = await prisma.integrationSettings.create({
    data: {
      integrationType: 'google',
      providerName: 'Google Calendar & Meet',
      config: {
        apiKey: 'AIzaSyDHGGOOWQxCvrySSBbXIx_vXz_FsDQL_jo',
        clientId: '841400225555-hqduasp5b451t6m8n5kc4t5gpg0eb0a0.apps.googleusercontent.com',
        clientSecret: '',
        redirectUri: ''
      },
      isActive: false // Initially inactive until user configures
  ...
  const smsIntegration = await prisma.integrationSettings.create({
    data: {
      integrationType: 'sms',
      providerName: 'Twilio',
      config: {
